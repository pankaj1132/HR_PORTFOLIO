...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <button
        className="fixed top-6 left-6 z-50 text-white text-3xl focus:outline-none"
      ...
      <aside
        className={`w-72 h-full bg-[#121212] flex flex-col fixed justify-between border-4 border-gray-900 px-10 py-12 z-40 transition-transform duration-300
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div>
          <div className="flex items-center space-x-2 mb-20">
            <img alt="logo" className="w-8 h-8" height="32" src={logo} width="32" />
            <span className="font-extrabold text-white text-lg select-none">
              QuickHire
            </span>
          </div>
          <nav className="flex flex-col space-y-6 text-sm font-normal">
            <Link
              className="text-[#4efb9f] font-semibold hover:text-[#4efb9f] transition cursor-pointer"
              to="home"
              smooth={true}
              duration={500}
              spy={true}
            ...
            <Link
              className="hover:text-[#4efb9f] transition cursor-pointer"
              to="about"
              smooth={true}
              duration={500}
              spy={true}

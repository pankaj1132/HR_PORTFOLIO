...
const FAQ = () => {
	...
	return (
		<section className="bg-[#121212] min-h-[80vh] py-16 px-4 flex flex-col items-center animate-fadeInUp">
			<Fade
				direction="up"
				triggerOnce
				cascade
			...
			>
				<h2 className="text-4xl font-extrabold text-[#4efb9f] mb-8 drop-shadow-lg text-center">
				...
				<div className="w-full max-w-2xl mx-auto">
					{faqs.map((faq, idx) => (
						...
						>
							<div
								className={`mb-4 rounded-xl shadow-lg border border-[#333] bg-[#23272a] transition-transform duration-300 hover:scale-[1.02] group ${
									openIndex === idx ? 'ring-2 ring-[#00ffcc]' : ''
								}`}
							>
								<button
									className="w-full flex justify-between items-center px-6 py-5 text-lg font-semibold text-left text-[#4efb9f] focus:outline-none transition-colors duration-200 group-hover:text-[#00ffcc]"
								...
								>
									...
									<span
										className={`ml-4 transform transition-transform duration-300 ${
											openIndex === idx
												? 'rotate-180 text-[#00ffcc]'
												: 'rotate-0 text-[#4efb9f]'
										}`}
									>
										▼
									</span>

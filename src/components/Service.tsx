...
const Service = () => {
  return (
    <motion.section
      className="py-20 px-4 bg-[#121212] min-h-screen"
    ...
    >
      <div className="max-w-6xl mx-auto text-center mb-12">
        <motion.h2 className="text-4xl sm:text-5xl font-extrabold text-white mb-4 tracking-tight">
          Our Services – <span className="text-[#4efb9f]">QuikHR</span>
        </motion.h2>
        <motion.p className="text-lg text-[#00ffcc] font-semibold mb-2">
          ...
          >
            ...
            <ul className="text-gray-300 text-sm space-y-2 pl-4 list-disc">
              {service.points.map((point, i) => (
                <li key={i} className="hover:text-[#00ffcc] transition-colors duration-200">{point}</li>
              ))}
            </ul>
          </motion.div>
        ))}
